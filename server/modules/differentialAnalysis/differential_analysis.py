...
class DifferentiatingFactorAnalysis:
    ...
    def __init__(self, 
                 ...
                 featureimportance_finetune:bool = True
                ):
        """
        ...
        self.X.columns = list(map(lambda x: self.feature_map[x], self.X.columns)) # we map features as certain may contain unsupported chars {/,\,%, etc}
        ...
        if ("xgb_feature_importance" in self.analyses) or ("randomforest_feature_importance" in self.analyses) or ("permutation_feature_importance" in self.analyses):
            self.FeatureImportance_Analyzer = FeatureImportance_Analysis(X=self.X,y=self.y, test_size = test_size, feature_type = feature_type, top_features_to_plot = top_features_to_plot, feature_map_reverse = self.feature_map_reverse,
                                                                         model_finetune = featureimportance_finetune, fine_tune_cv_nfolds = n_folds, scoring = scoring, outdir = outdir)
    ...
    def perform_randomforest_feature_importance_analysis(self):
        ...
        rf_importances = self.FeatureImportance_Analyzer.RandomForestFeatureImportance()
        self.top_features["randomforest_feature_importance"] = rf_importances

        # Prepare data for plotting
        sorted_features = sorted(rf_importances.items(), key=lambda item: item[1], reverse=True)
        top_features_df = pd.DataFrame(sorted_features[:self.top_features_to_plot], columns=['Features', 'Importance'])

        # Plot the importances
        logging.info("Plotting Random Forest Feature Importances")
        plt.figure(figsize=(30, 45))
        colors = ['blue'] * len(top_features_df) # Or use a colormap
        plt.barh(top_features_df['Features'], top_features_df['Importance'], color=colors)
        plt.xlabel('Importance Score', fontsize=15)
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        plt.title(f'Random Forest Feature Importance of Top {self.top_features_to_plot} Differentiating {self.feature_type}s', 
                  fontsize=25, loc='center', pad=15)
        plt.gca().invert_yaxis()
        
        # Save the plot
        logging.info("Saving Plots")
        plot_path_png = f'{self.outdir}/feature_importance/png/randomforest_feature_importance_plot.png'
        plot_path_pdf = f'{self.outdir}/feature_importance/pdf/randomforest_feature_importance_plot.pdf'
        plt.savefig(plot_path_png, bbox_inches='tight')
        plt.savefig(plot_path_pdf, bbox_inches='tight')
        print(f"Saved Random Forest plot: {plot_path_png}")
    ...
    def perform_xgb_feature_importance_analysis(self):
        ...
        # Calculate feature importances
        xgb_importances = self.FeatureImportance_Analyzer.XGBoostFeatureImportance()
        self.top_features["xgb_feature_importance"] = xgb_importances
                
        # Prepare data for plotting
        sorted_features = sorted(xgb_importances.items(), key=lambda item: item[1], reverse=True)
        top_features_df = pd.DataFrame(sorted_features[:self.top_features_to_plot], columns=['Features', 'Importance'])

        # Plot the importances
        logging.info("Plotting XGBoost Feature Importances")
        plt.figure(figsize=(30, 45))
        colors = ['blue'] * len(top_features_df) # Or use a colormap
        plt.barh(top_features_df['Features'], top_features_df['Importance'], color=colors)
        plt.xlabel('Importance Score (F-Score or Gain)', fontsize=15) # Note: XGBoost importance type can vary
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        plt.title(f'XGBoost Feature Importance of Top {self.top_features_to_plot} Differentiating {self.feature_type}s', 
                  fontsize=25, loc='center', pad=15)
        plt.gca().invert_yaxis()
        
        # Save the plot
        logging.info("Saving Plots")
        plot_path_png = f'{self.outdir}/feature_importance/png/xgb_feature_importance_plot.png'
        plot_path_pdf = f'{self.outdir}/feature_importance/pdf/xgb_feature_importance_plot.pdf'
        plt.savefig(plot_path_png, bbox_inches='tight')
        plt.savefig(plot_path_pdf, bbox_inches='tight')
        print(f"Saved XGBoost plot: {plot_path_png}")
    ...
    def perform_permutation_feature_importance_analysis(self):
        ...
        # Filter out features with non-positive importance for plotting clarity if desired
        # sorted_features = [item for item in sorted_features if item[1] > 0] 
        top_features_df = pd.DataFrame(sorted_features[:self.top_features_to_plot], columns=['Features', 'Importance Decrease'])

        # Plot the importances
        logging.info("Plotting Permutation Feature Importances")
        plt.figure(figsize=(30, 45))
        colors = top_features_df['Importance Decrease'].apply(lambda x: 'red' if x < 0 else 'blue') # Color negative importance red
        plt.barh(top_features_df['Features'], top_features_df['Importance Decrease'], color=colors)
        plt.xlabel('Mean Importance Decrease', fontsize=15)
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        plt.title(f'Permutation Feature Importance of Top {self.top_features_to_plot} Differentiating {self.feature_type}s', 
                  fontsize=25, loc='center', pad=15)
        plt.gca().invert_yaxis() # Highest importance at the top
        
        # Save the plot
        logging.info("Saving Plots")
        plot_path_png = f'{self.outdir}/feature_importance/png/permutation_feature_importance_plot.png'
        plot_path_pdf = f'{self.outdir}/feature_importance/pdf/permutation_feature_importance_plot.pdf'
        plt.savefig(plot_path_png, bbox_inches='tight')
        plt.savefig(plot_path_pdf, bbox_inches='tight')
        print(f"Saved Permutation Importance plot: {plot_path_png}")
    ...
    def run_all_analyses(self):
        ...
        # Save the feature importances JSON file organized by class pairs
        json_path = f"{self.outdir}/feature_importances.json"
