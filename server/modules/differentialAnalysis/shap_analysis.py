...
class SHAP_Analysis:
    ...
    def shapWaterFall(self):
        ...
        shap.plots.waterfall(self.shap_values[self.random_samples[class_names[0]]], 
                              max_display=self.top_features_to_plot, show=False)
        
        # Increase baseline text box size and font
        for text in axes[0].texts:
            if "base value" in text.get_text().lower():
                text.set_fontsize(18)  # Increase font size
                text.set_bbox(dict(facecolor='blue', alpha=0.8, edgecolor='grey', 
                               boxstyle='round,pad=0.8', linewidth=100))  # Increase box width
        
        axes[0].set_title(f"{class_names[0]} Sample", fontsize=15)
        
        # Waterfall plot for the second sample
        plt.sca(axes[1])  # Set the current axis to the second subplot
        shap.plots.waterfall(self.shap_values[self.random_samples[class_names[1]]], 
                              max_display=self.top_features_to_plot, show=False)
        
        axes[1].set_title(f"{class_names[1]} Sample", fontsize=15)
        
        # Adjust space between plots
        plt.subplots_adjust(wspace=3.0)
        plt.tight_layout(pad=2.0)

        # Add dynamic title to the figure
        plt.suptitle(f"Waterfall Plots for {class_names[0]} and {class_names[1]} Samples", fontsize=20, y=1.02, x=0.55)
        logging.info("Saving Plots")
        plt.savefig(f"{self.outdir}/png/shap_waterfall_subplots_{class_names[0]}_and_{class_names[1]}.png", bbox_inches='tight')
        plt.savefig(f"{self.outdir}/pdf/shap_waterfall_subplots_{class_names[0]}_and_{class_names[1]}.pdf", bbox_inches='tight')
        plt.close()
        print(f"{self.outdir}/png/shap_waterfall_subplots_{class_names[0]}_and_{class_names[1]}.png")
    ...
    def shapForce(self):
        ...
        # Set general font size for plots
        plt.rcParams.update({'font.size': 25})
        
        plt.figure(figsize=(15, 5))
        # Plot for class 0
        shap.plots.force(self.shap_values[self.random_samples[class_names[0]]], matplotlib=True, show=False, figsize=(35, 5))
        
        plt.title(f"Force Plot for 1 {class_names[0]} Sample", y=1.5, fontsize=30)
        plt.tight_layout()
        ...
        plt.close()
        print(f"{self.outdir}/png/forceplot_for_{class_names[0]}_sample.png")
        # Plot for class 1
        shap.plots.force(self.shap_values[self.random_samples[class_names[1]]], matplotlib=True, show=False, figsize=(35, 5))
        plt.title(f"Force Plot for 1 {class_names[1]} Sample", y=1.5, fontsize=30)
        plt.tight_layout()
        logging.info("Saving Plots")
        ...
        plt.close()
        print(f"{self.outdir}/png/forceplot_for_{class_names[1]}_sample.png")
    ...
    def shapSummary(self):
        ...
        logging.info("Saving Plots")
        plt.savefig(f'{self.outdir}/png/shap_summary_plot_{class_names[0]}_and_{class_names[1]}.png', bbox_inches='tight')
        plt.savefig(f'{self.outdir}/pdf/shap_summary_plot_{class_names[0]}_and_{class_names[1]}.pdf', bbox_inches='tight')
        print(f'{self.outdir}/png/shap_summary_plot_{class_names[0]}_and_{class_names[1]}.png')
        #plt.show()
    ...
    def shapHeatmap(self):
        ...
        # Double the figure height (from previous 18x10 to now 18x20)
        plt.figure(figsize=(18, 40), dpi=300)
        
        # Set higher heatmap height
        ...
        logging.info("Saving Plots")
        plt.savefig(f'{self.outdir}/png/shap_heatmap_plot_{class_names[0]}_and_{class_names[1]}.png', bbox_inches='tight')
        plt.savefig(f'{self.outdir}/pdf/shap_heatmap_plot_{class_names[0]}_and_{class_names[1]}.pdf', bbox_inches='tight')
        print(f'{self.outdir}/png/shap_heatmap_plot_{class_names[0]}_and_{class_names[1]}.png')
        plt.close()
    ...
    def meanSHAP(self):
        ...
        logging.info("Saving Plots")
        plt.savefig(f'{self.outdir}/png/mean_shap_plot_{class_names[0]}_and_{class_names[1]}.png', bbox_inches='tight')
        plt.savefig(f'{self.outdir}/pdf/mean_shap_plot_{class_names[0]}_and_{class_names[1]}.pdf', bbox_inches='tight')
        print(f'{self.outdir}/png/mean_shap_plot_{class_names[0]}_and_{class_names[1]}.png')
        #plt.show()
    ...
    def shapFeatureImportance(self):
        ...
        # Changed here. Original: float(shap_importance[i])
        top_features = {shap_importance.index[i]: float(shap_importance.iloc[i]) for i in range(len(shap_importance))}
