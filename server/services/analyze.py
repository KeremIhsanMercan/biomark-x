...
featureimportance_finetune = False  # Model fine-tuning for feature importance analysis
...
def initial_visualization(data, visualizations, outdir, selectedSampleColumn):
...
def feature_selection(outdir):
    ...
    # Perform feature ranking for each class pair
    all_top_features = []
    
    # If JSON is in new format (with class pairs)
    if feature_importances and isinstance(feature_importances, dict) and any(isinstance(value, dict) and any(isinstance(inner_value, dict) for inner_value in value.values()) for value in feature_importances.values()):
        # Use new format (organized by class pairs)
        top_n = feature_rank(top_features=feature_importances, num_top_features=num_top_features, feature_type=feature_type, outdir=outdir)
    else:
        # Legacy format - for backward compatibility
        # In this case, JSON is assumed to be a dict of analyses directly
        legacy_format = {"default_class_pair": feature_importances}
        top_n = feature_rank(top_features=legacy_format, num_top_features=num_top_features, feature_type=feature_type, outdir=outdir)
    
    return top_n

# Post-feature selection visualization function
def visualization_after_feature_selection(data, visualizations, outdir):
    if(visualizations != ['']):
...
if __name__ == "__main__":
    ...
    if params_json:
        try:
            ...
            if "featureimportance_finetune" in params_json:
                globals()["featureimportance_finetune"] = params_json["featureimportance_finetune"]
    ...
    print(f"featureimportance_finetune: {featureimportance_finetune}")
    ...
    if visualizations and visualizations != ['']:
        ...
        initial_visualization(data, visualizations, outdir, selectedSampleColumn)
    ...
    if os.path.exists(feature_importances_file_path):
        print("feature_selection function.....\n")
        feature_selection(outdir)

        # Feature selection
        top_n = feature_selection(outdir)
        data = data[[selectedIllnessColumn] + top_n]
        
        # Post-feature selection visualization
        if visualizations and visualizations != ['']:
            print("visualization_after_feature_selection function.....\n")
            visualization_after_feature_selection(data, visualizations, outdir)

        # Post-feature selection model training
        if model_list and model_list != ['']:
            print("model_training_after_feature_selection function.....\n")
            model_training_after_feature_selection(data, selectedIllnessColumn, outdir)
