...
function App() {
  ...
  const [isDiffAnalysis, setIsDiffAnalysis] = useState([]); // Stores differential analysis state
  ...
  const [summaryImagePath, setSummaryImagePath] = useState(null);
    ...
    try {
      ...
      if (response.data && response.data.filePath) {
        ...
        setUploadedInfo({
          ...
          size: "3 MB", // This may be static info
  ...
  const handleUpload = async () => {
    ...
    if (!isValidType && file.name) {
        ...
        if (fileExtension === 'csv' || fileExtension === 'gz' || fileExtension === 'tsv') { // Assume .tsv is added
            isValidType = true;
        }
    ...
    if (!isValidType) {
        setError('Please upload a valid file (CSV, TSV, or GZ).');
  ...
  const handleIllnessColumnSelection = async (illnessColumn) => {
    ...
    if (uploadedInfo?.filePath) {
        try {
          setLoading(true); // Loading state while fetching classes
  ...
  }, [selectedIllnessColumn, selectedSampleColumn]);
  ...
  const handleAnalysisSelection = async (selectedAnalyzesUpdate) => {
    ...
    if (differential.length > 0) {
      setIsDiffAnalysis(differential); // This state may not be necessary, as it's already in selectedAnalyzes
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (response.data.imagePath) {
          ...
          // Update or add new summary for the same class pair and feature count
          setSummarizeAnalyses(prev => {
              const existingIndex = prev.findIndex(s => s.classPair === newSummary.classPair && s.featureCount === newSummary.featureCount);
              if (existingIndex > -1) {
  ...
  }, [showStepThree, showStepFour, showStepFive, selectedIllnessColumn, selectedSampleColumn, selectedClasses, showStepSix, showStepAnalysis, analyzing]);
      ...
      <div className="file-browse-section">
        ...
        <div className="file-input-container">
          ...
          <div className="format-instructions-row">
            <a href="#" className="format-instructions-link" onClick={(e) => {
              e.preventDefault();
        ...
        <div key={id}>
          ...
          {index === anotherAnalysis.length - 1 && (
            <>
                  ...
                  <div className="column-select-block">
                    <label>Patient Group Column:</label>
                      <SearchableColumnList
                        ...
                        disabled={loadingAllColumns || loading}
                      />
                  </div>
              ...
              {/* Step 4: Class Table */}
              {classTable.class && Array.isArray(classTable.class) && showStepFour && classTable.class.length > 0 && (
              ...
              {showStepFour && selectedClasses && selectedClasses.length > 0 && (
            ...
            <>
              <div className="show-analysis-results">
                {analysisInformation[index] && (
                  <>
                  {(() => {
                    const shouldShowInfoBlock = index === 0 || 
                    ...
                    // Check if file and class information are identical
                            ...
                            <>
                              <div className="analysis-detail">
                                ...
                                <span className="detail-value">
                                  {analysisInformation[index].filePath?.split('/').pop() || "N/A"}
                                </span>
                              </div>
                              <div className="analysis-detail">
              ...
              {index === anotherAnalysis.length - 1 && (          
                <div className="post-analysis-options">
                  ...
                  <div className="feature-count-selector">
                    <label htmlFor="featureCount">Number of most influential miRNAs to display: </label>
                    <select 
                      id="featureCount" 
