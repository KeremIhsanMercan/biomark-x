...
const AnalysisReport = ({ 
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const pageHeight = Math.max(297, contentHeight * 0.347); // Content height + more space
      ...
      if (summarizeAnalyses && summarizeAnalyses.length > 0) {
        ...
        for (let k = 0; k < summarizeAnalyses.length; k++) {
          ...
          try {
            // Find the corresponding image in HTML (improve selector if needed)
            const imgElement = document.querySelector(`#analysis-report .summary-section[data-classpair="${summaryAnalysis.classPair}"] .summary-image img`);

            if (imgElement) {
              const canvas = await html2canvas(imgElement, { 
                scale: 2, // Better resolution
                useCORS: true, // If needed
                logging: false, // Disable console logs
                allowTaint: true, // For cross-origin images
                imageTimeout: 15000, // Loading timeout
                removeContainer: true, // Remove temp container
                backgroundColor: '#ffffff', // Background color (important for JPEG)
              });
              
              const imgData = canvas.toDataURL('image/jpeg', 0.8); // JPEG and compression
              ...
              yPosition += imgHeight + 15;
                ...
                pdf.text(`*Summary image for ${summaryAnalysis.classPair} could not be rendered.`, marginLeft, yPosition);
                yPosition += 10;
            }
          } catch (error) {
